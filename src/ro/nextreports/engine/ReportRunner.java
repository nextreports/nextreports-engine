...
import ro.nextreports.engine.exporter.JSONSimpleExporter;
...
public class ReportRunner implements Runner {
	...
	public static final String[] FORMATS = { PDF_FORMAT, EXCEL_FORMAT, EXCEL_XLSX_FORMAT, HTML_FORMAT, DOCX_FORMAT,
			RTF_FORMAT, CSV_FORMAT, TSV_FORMAT, TXT_FORMAT, XML_FORMAT, JSON_SIMPLE_FORMAT, JSON_FULL_FORMAT };
	...
	// to write excel in a existing template, in a specific sheet (other sheets
	// may contain calculations on data sheet)
	 ...
	 *            database connection
	 * @param csv
	 *            true for a csv file connection
	 */
	...
	private void createExporter(ExporterBean bean) {
		...
		} else if (JSON_SIMPLE_FORMAT.equals(format)) {
			exporter = new JSONSimpleExporter(bean);
		} else if (JSON_FULL_FORMAT.equals(format)) {
			exporter = new JSONFullExporter(bean);
		} else {
	 ...
	 * 
	 * Language string is computed as : locale.getLanguage() + "_" +
	 * locale.getCountry()
	 * 
	 * @param language

...
public class QueryParameter implements Serializable {
	...
	/**
	 * Parameter multiple selection : more than one value can be selected at
	 * runtime
	 */
	...
	// These values are used in the orderByCombo from SourceDialog (do not
	// modify)
	...
	public static final String[] ALL_VALUES = { INTEGER_VALUE, STRING_VALUE, BOOLEAN_VALUE, BYTE_VALUE, DATE_VALUE,
			TIMESTAMP_VALUE, TIME_VALUE, DOUBLE_VALUE, FLOAT_VALUE, LONG_VALUE, SHORT_VALUE, BIGDECIMAL_VALUE,
			BIGINTEGER_VALUE, OBJECT_VALUE };
	...
	/**
	 * public no-arg constructor added only for JAXB marshall/unmarshall
	 */
	public QueryParameter(){
		this.name = "__default__";
		this.description = "__default__";
		this.valueClassName = "__default__";
	}
	 ...
	 * Set parameter name
	 * 
	 * This method is useful if we want to clone a parameter because we need to
	 * change the name
	 */
	 ...
	 *            true if this parameter is a procedure parameter, false
	 *            otherwise
	 */
	...
	/**
	 * Get preview value for procedure parameter This value must be used inside
	 * the business procedure to exit (so we can can the columns as fast as
	 * possible)
	 *
	 ...
	 *
	 * @param previewValue
	 *            preview value for procedure parameter
	 */
	...
	/**
	 * Get order for parameter values : ORDER_BY_NAME, ORDER_BY_ID Has meaning
	 * only if parameter has a manual source
	 *
	 ...
	 *
	 * @param orderBy
	 *            one of ORDER_BY_NAME, ORDER_BY_ID
	 */
	...
	/**
	 * See if parameter is dependent on other parameters (its manual source
	 * contains other parameters names between tags '${' and '}'
	 * 
	 * @return true if parameter is dependent on other parameters, false
	 *         otherwise
	 */
	 ...
	 *
	 * @return a list of the names of all parameters the current parameter
	 *         depends on
	 */
	 ...
	 * @param o
	 *            parameter object
	 * @return true if current parameter object equals parameter object, false
	 *         otherwise
	 */
	public boolean equals(Object o) {
		...
		if (schema != null ? !schema.equals(that.schema) : that.schema != null)
			return false;
		if (orderBy != that.getOrderBy())
			return false;

		if (source != null ? !source.equals(that.source) : that.source != null)
			return false;
		if (defaultSource != null ? !defaultSource.equals(that.defaultSource) : that.defaultSource != null)
			return false;
	...
	private boolean basicEquals(Object o) {
		...
		QueryParameter that = (QueryParameter) o;
		if (manualSource != that.manualSource)
			return false;
		...
		if (valueClassName != null ? !valueClassName.equals(that.valueClassName) : that.valueClassName != null)
			return false;
		if (isProcedureParameter != that.isProcedureParameter)
			return false;
		if (previewValue != null ? !previewValue.equals(that.previewValue) : that.previewValue != null)
			return false;
		if (defaultValues != null && that.defaultValues != null
				&& (!defaultValues.containsAll(that.defaultValues) || !that.defaultValues.containsAll(defaultValues)))
			return false;
	...
	public boolean compare(Object o) {
		...
		// compare sources to have the same select no matter of letter-case and
		// white spaces
		if (!StringUtil.equalsText(source, that.source))
			return false;
		if (!StringUtil.equalsText(defaultSource, that.defaultSource))
			return false;
	...
	public String toString() {
		return "QueryParameter{" + "name='" + name + '\'' + ", runtimeName='" + runtimeName + '\'' + ", description='"
				+ description + '\'' + ", valueClassName='" + valueClassName + '\'' + ", source='" + source + '\''
				+ ", selection='" + selection + '\'' + ", ignore=" + ignore + ", mandatory=" + mandatory
				+ ", manualSource=" + manualSource + ", schema='" + schema + '\'' + ", isProcedureParameter="
				+ isProcedureParameter + ", previewValue='" + previewValue + '\'' + ", orderBy=" + orderBy + ", hidden="
				+ hidden + ", defaultValues=" + defaultValues + ", defaultSource=" + defaultSource
